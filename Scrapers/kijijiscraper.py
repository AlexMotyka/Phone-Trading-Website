...
def geturllist(firstpage=None,testing=False,html=None,):
    ...
    if testing:
        page = BeautifulSoup(html, 'lxml') #load it in BS
    else:
    ...
    url = url.split('2',1)
    pagesurl += map(lambda x: url[0]+str(x)+url[1],xrange(3,10))

    return pagesurl

# create a list of all Item urls
def getitemurl(url=None,testing=False, html=None):
...
def iteminfo(url=None,testing=False,html=None):
